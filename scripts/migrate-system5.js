// Migration script for System 5 historical data
// Win market only
...
const CSV_FILE_PATH = path.join(
  __dirname,
  "../system-historical-data/FST System 5.xlsx - FST System 5.csv"
);
...
async function migrateSystem5() {
  try {
    console.log("🔄 Starting System 5 migration...");
    await connectDB();
    console.log("✅ Connected to database");

    let system5 = await System.findOne({ slug: "system-5" });
    if (!system5) {
      system5 = await System.create({
        name: "System 5",
        slug: "system-5",
        isActive: true,
      });
      console.log("✅ Created System 5");
    } else {
      console.log("✅ Found existing System 5");
    }

    const deleted = await SystemSelection.deleteMany({ systemId: system5._id });
