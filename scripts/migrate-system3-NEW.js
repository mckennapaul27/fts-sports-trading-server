// Migration script for NEW System 3 historical data
...
const CSV_FILE_PATH = path.join(
  __dirname,
  "../system-historical-data/FST New System 3.xlsx - FST New System 3.csv"
);
...
async function migrateSystem3New() {
  try {
    console.log("🔄 Starting NEW System 3 migration...");

    // Connect to database
    await connectDB();
    console.log("✅ Connected to database");

    // Find System 3 (should already exist)
    const system3 = await System.findOne({ slug: "system-3" });
    if (!system3) {
      throw new Error(
        "System 3 not found in database. Please create it first with uniqueName."
      );
    }
    console.log(`✅ Found System 3: ${system3.name} (${system3.uniqueName})`);

    // Delete existing System 3 selections to avoid duplicates
    const deletedCount = await SystemSelection.deleteMany({
      systemId: system3._id,
    });
