// Migration script for System 8 historical data
// Win market only
...
const CSV_FILE_PATH = path.join(
  __dirname,
  "../system-historical-data/FST System 8.xlsx - FST System 8.csv"
);
...
async function migrateSystem8() {
  try {
    console.log("🔄 Starting System 8 migration...");
    await connectDB();
    console.log("✅ Connected to database");

    let system8 = await System.findOne({ slug: "system-8" });
    if (!system8) {
      system8 = await System.create({
        name: "System 8",
        slug: "system-8",
        isActive: true,
      });
      console.log("✅ Created System 8");
    } else {
      console.log("✅ Found existing System 8");
    }

    const deleted = await SystemSelection.deleteMany({ systemId: system8._id });
